<?php
/*
 * 企业微信SDK入口类(SDK: 采用ioc容器,入口类做分发调用操作)
 *   
...
use \weworkapi\api\datastructure\Agent;
use \weworkapi\api\datastructure\Department;
use \weworkapi\api\datastructure\tag\Tag;
use \weworkapi\api\datastructure\batch\Batch;
use \weworkapi\api\datastructure\batch\BatchJobArgs;
use \weworkapi\api\datastructure\menu\Menu;
use \weworkapi\api\datastructure\message\Message;
use \weworkapi\api\datastructure\oauth\UserInfoByCode;
use \weworkapi\api\datastructure\oauth\UserDetailByUserTicket;
use \weworkapi\api\datastructure\user\User;
use \weworkapi\api\datastructure\checkinoption\CheckinOption;
use \weworkapi\api\datastructure\checkindata\CheckinDataList;
use \weworkapi\api\datastructure\approvaldata\ApprovalDataList;
use \weworkapi\api\datastructure\external\External;
use \weworkapi\api\datastructure\external\FollowUser;
use \think\Cache;
use \think\Config;
...
class CorpAPI extends API
{
    ...
    {
        if ( ! Utils::notEmptyStr($this->accessToken)) {      //jiang.ding注释
//        if ( !Cache::get($this->cacheKey['access_token'])) {    //jiang.ding新增
...
//        return Batch::Array2BatchJobResult($this->rspJson,$jobId?:'');    //jiang.ding改
   ...
   // ------------------------- jiang.ding 自定义部分 --------------------------------------
    ...
    /**
     * 获取wx.Agentconfig的ticket
     * @param array $args GET请求的url参数+值
     * @return string
     * @throws QyApiError
     */
    public function agentTicketGet($args = array('type'=>'agent_config')) {
        self::_HttpCall(self::GET_TICKET, 'GET',$args);
        return $this->rspJson["ticket"];
    }

    /**
     * 获取外部联系人详情
     * @param int $externalUserid 外部联系人ID
     * @return array 外部联系人详情
    ...
    public function getExternalContact($externalUserid) {
        ...
        self::_HttpCall(self::GET_EXTERNAL_CONTACT, 'GET', array('external_userid' => $externalUserid)); 
        $externalObj = External::Array2User($this->rspJson['external_contact']);
        $followUserObj = FollowUser::Array2User($this->rspJson['follow_user']);
        $externalInfo = Utils::Object2Array($externalObj); 
        $followUserList = Utils::Object2Array($followUserObj); 
        return array_merge($externalInfo,$followUserList);
    }
    
    /**
     * 获取企业微信企业成员code的跳转地址
     * @param array $args uri参数
     * @return object(json) 获取用户信息 userid+deviceid+user_ticket    
     */
    public  function getCodeRedirectUrl(array $args) {  
        $requestData = [
            'appid' => Config::get('wework.CORP_ID'),  //企业corpId
            'redirect_uri' => urlencode($args['redirectUrl']),  //回调页面url
            'response_type' => 'code',
            'scope' => empty($args['scope']) ? 'snsapi_base' : $args['scope'] , // 静默授权
            'agentid'=> Config::get('WEWORK_AGENT_ID'), // 当scope!=snsapi_base时,则agentId为必填项
        ];
        !empty($requestData['state']) && $requestData['state'] = $args['state'];    // 重定向后会带上state参数，企业可以填写a-zA-Z0-9的参数值
        $redirectUrl = 'https://open.weixin.qq.com/connect/oauth2/authorize?'. http_build_query($requestData) .'#wechat_redirect';   //获取企业成员code       
        return $redirectUrl;
    }
